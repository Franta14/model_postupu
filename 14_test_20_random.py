...
try:
    ...
    print("âťŚ Soubory v cache nenalezeny. Spustte nejprve setup_mapa.py")
    sys.exit(1)
...
try:
    ...
    print(f"đź“Ź NaÄŤteno {len(road_lines_grid)} vektorovĂ˝ch os cest pro vizuĂˇlnĂ­ snap.")
except FileNotFoundError:
    print("âš ď¸Ź Vektory cest nenalezeny, snap nebude aktivnĂ­. SpusĹĄte setup_mapa.py.")
...
print("đź—şď¸Ź Extrahuji validnĂ­ kontrolnĂ­ body z OMAP...")
...
POINT_SYMBOLS = {
    "109", "110", "111", "112", "115", # TerennĂ­ tvary (kupky, jĂˇmy)
...
print(f"âś… Nalezeno {len(valid_points)} bodĹŻ ve sjĂ­zdnĂ©m terĂ©nu.")

# 3. Kresleni do mapy
print("đź–Ľď¸Ź NaÄŤĂ­tĂˇm PNG mapu pro vykreslovĂˇnĂ­...")
...
def draw_leg_image(p1, p2, routes, filename):
    ...
    for i, route in enumerate(routes):
        ...
        # PĹ™evedeme trasu (v pixelech gridu) na pixely obrĂˇzku
        img_route = [grid_to_img(gy, gx) for gy, gx in route]
        
        if len(img_route) > 1:
            draw.line(img_route, fill=color, width=8, joint='curve')
            
    # 2. Spojnice (prerusenĂˇ ÄŤĂˇra, pĹ™eruĹˇenĂ­ blĂ­zko koleÄŤek)
    # NakreslĂ­me ÄŤĂˇru, ale zkrĂˇtĂ­me ji o radius koleÄŤka na obou koncĂ­ch
...
# ================================================================
# 4. MASIVNĂŤ GENEROVĂNĂŤ POSTUPĹ® â€“ PLNĂ‰ POKRYTĂŤ MAPY
# ================================================================
import time
...
def is_viable_pair(p1, p2):
    """OdmĂ­tne pĂˇry kde >40% pĹ™Ă­mky je neprĹŻchodnĂˇ (zdi, voda)."""
    n = 10
    impassable = 0
...
def vzdalenost_m_ctrl(c1, c2):
    """VzdĂˇlenost dvou kontrol v metrech."""
...
def jsou_podobne(pos1, pos2):
    """Dva postupy jsou podobnĂ© pokud majĂ­ blĂ­zkĂ© starty+cĂ­le a podobnou dĂ©lku."""
...
# â”€â”€ FĂZE 1: GenerovĂˇnĂ­ kandidĂˇtnĂ­ch pĂˇrĹŻ â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

print("VytvĂˇĹ™Ă­m mapu pĹ™Ă­rodnĂ­ho vlnÄ›nĂ­ (low-frequency noise)...")
np.random.seed(42) # PevnĂ˝ seed pro konzistenci vlnÄ›nĂ­ mezi bÄ›hy
...
# Ĺ kĂˇlovĂˇnĂ­ na multiplier [0.97, 1.03] (+- 3%) pro rozbitĂ­ pĹ™Ă­mek
...
# StratifikovanĂ˝ vĂ˝bÄ›r: rovnomÄ›rnÄ› z kaĹľdĂ©ho rozsahu
...
# DoplnÄ›nĂ­ zbĂ˝vajĂ­cĂ­ch slotĹŻ z pĹ™ebytku
...
# â”€â”€ FĂZE 2: Dijkstra analĂ˝za + skĂłre zajĂ­mavosti â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
for idx, (p1, p2, dist_m) in enumerate(candidates):
    ...
    # SkĂłre zajĂ­mavosti
    # 1. Divergence: symetrickĂˇ prĹŻmÄ›rnĂˇ nepodobnost
    pairwise_sims = []
    ...
    # 2. VyvĂˇĹľenost: pomÄ›r nejrychlejĹˇĂ­/nejpomalejĹˇĂ­
...
# â”€â”€ FĂZE 3: ChytrĂ˝ vĂ˝bÄ›r (deduplikace) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ FĂZE 4: Export â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
print(f"Faze 4/4: Ukladam {len(selected)} postupu...")

# Smazat starĂ© soubory
old_count = 0
...
for i, postup in enumerate(selected):
    ...
    # Snap na vektorovĂ© osy cest (ÄŤistÄ› vizuĂˇlnĂ­)
    routes_viz = []
